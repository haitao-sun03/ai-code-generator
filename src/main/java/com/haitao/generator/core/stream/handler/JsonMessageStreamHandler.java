    ...
    public Flux<String> handle(Flux<String> originStream, ChatHistoryService chatHistoryService, long appId, long userId) {
        ...
        return originStream
                ...
                .doOnError(error -> {
                    ...
                    chatHistoryService.addChatMessage(appId, errorMessage, ChatMessageTypeEnum.AI.getValue(), userId);
                });
    }

    /**
     * 解析并收集上游的Json数据（由TokenStream解析封装）
     */
    private String handleJsonMessageChunk(String chunk, StringBuilder chatHistoryStringBuilder, Set<String> seenToolIds) {
        // 解析 JSON
        StreamMessage streamMessage = JSONUtil.toBean(chunk, StreamMessage.class);
        StreamMessageTypeEnum streamMessageTypeEnum = StreamMessageTypeEnum.getEnumByValue(streamMessage.getType());
        switch (streamMessageTypeEnum) {
            case AI_TEXT_RESPONSE -> {
                AiResponseTextMessage aiMessage = JSONUtil.toBean(chunk, AiResponseTextMessage.class);
